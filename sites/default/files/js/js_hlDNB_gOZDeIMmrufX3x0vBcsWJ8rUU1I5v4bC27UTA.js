...
(function ($) {
  $(document).ready(function(){
    ...
    $('.view-featured-events .view-content').owlCarousel({
      items: 4,
      itemsCustom: [[0,2], [768,3], [1140,4]],
   //  afterAction: owlCenterItems,
    });

    // Carousel for category event groups.
    $('.view-events-by-categories .view-upcoming-events-from-term').each(function(){
      if ($(this).find('.view-content').length) {
        // Convert content to carousel.
        $(this).find('.view-content').owlCarousel({
          items: 5,
          itemsCustom: [[0,2], [650,3], [1024,4], [1224,5]],
    //      afterAction: owlCenterItems,
        });

      } else {
        // If no view content, term must be empty, remove it.
        $(this).closest('.views-row').remove();
      }
    });

    // Carousel for location event groups.
    $('.view-events-by-locations .view-upcoming-events-from-term, .page-taxonomy-term-256 .view-upcoming-events-from-term').each(function(){
      if ($(this).find('.view-content').length) {
        // Convert content to carousel.
        $(this).find('.view-content').owlCarousel({
          items: 5,
  ...
  //        afterAction: owlCenterItems,
    ...
    setTimeout(function() {
      console.log('hi');
      $('.field-name-field-reservation-embed-code .switch_edit-field-reservation-embed-code-und-0-value').click();
    },1000);
